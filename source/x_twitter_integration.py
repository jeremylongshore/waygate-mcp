...
class XTwitterAPI:
    ...
    def _get_bearer_token(self) -> Optional[str]:
        ...
        if oauth_token:
            logger.warning(
                "Found OAuth 2.0 access token, but Bearer token is preferred for app-only auth"
            )
        ...
        logger.info(
            "💡 Consider using OAuth 1.0a for permanent authentication (see X_OAUTH1A_SETUP.md)"
        )
        return None
    ...
    def _get_auth_headers(self) -> Dict[str, str]:
        ...
        if not self.bearer_token:
            ...
            if self.oauth1a_available:
                error_msg += (
                    ". OAuth 1.0a available - consider using oauth1a tools instead"
                )
    ...
    async def post_tweet(self, text: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(text) > 280:
                raise MCPToolError(
                    f"Tweet text too long: {len(text)} characters (max 280)"
                )
            ...
            logger.info(
                f"Posting tweet: {text[:50]}..."
                if len(text) > 50
                else f"Posting tweet: {text}"
            )
    ...
    def _parse_error_message(self, status_code: int, error_data: Dict[str, Any]) -> str:
        ...
        if status_code == 401:
            ...
            return (
                f"API error {status_code}: {error_data.get('detail', 'Unknown error')}"
            )
...
async def post_tweet_tool(parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = await x_api.post_tweet(
        text=text, reply_to_id=reply_to_id, poll=poll, media_ids=media_ids
    )
        ...
        },
        {
            ...
            "parameters": {
                "username": {
                    "type": "string",
                    "required": False,
                    "description": "Username (omit for own info)",
                }
            },
